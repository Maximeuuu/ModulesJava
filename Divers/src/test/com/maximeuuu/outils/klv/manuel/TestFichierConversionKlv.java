...
import com.maximeuuu.outils.klv.conversion.misb.DecodeurMisb;
...
public class TestFichierConversionKlv {
	...
	private static void lireKLV(){
		...
		IDecodeurTlv decodeur = new DecodeurMisb();
			...
			while ((klv = scanner.next()) != null) {
				...
				if( valideur.estValide(klv, ensTlv) ){
					for( KlvEncode tlv : ensTlv ){
						KlvDecode tlvDecode = decodeur.decoder( tlv );
						System.out.println("  * " + tlvDecode.toString());
					}
				}
			}
		} catch (IOException e) {
